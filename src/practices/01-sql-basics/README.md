...
답안을 PostgreSQL 18에서 동적으로 확인할 때는 보존할 데이터가 없는 로컬 전용 데이터베이스를 따로 만들고 이름을 `codeit_prisma_practice_`로 시작합니다. 다음 값은 형식 예시이며 실제 비밀번호를 파일에 기록하지 않습니다. 사용하는 터미널에 맞는 명령 하나만 실행합니다.
