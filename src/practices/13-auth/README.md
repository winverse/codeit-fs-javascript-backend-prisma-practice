...
`authenticate(secrets, options)`는 비동기 Express 미들웨어를 반환합니다. Access Token과 Refresh Token의 payload에는 모두 `userId`만 넣습니다. `verifyToken()`은 정상 token의 payload를 반환하고, 지원하지 않는 token 종류나 누락·변조·만료된 token에는 오류를 외부로 던지지 않고 `null`을 반환합니다. `options.findUserById(userId)`로 Access Token의 사용자를 다시 조회하고, 조회한 사용자에서 `password`를 제거해 `req.user`에 저장합니다. Access Token이 없거나 유효하지 않거나 사용자가 없으면 401을 반환합니다.
